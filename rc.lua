...
run_once({ "caffeine","unclutter -root", "pa-applet", "light-locker", "xautolock -detectsleep -time 3 -locker \"light-locker-command -l\" -notify 30 -notifier \"notify-send -u critical -t 10000 -- 'Locking screening 30 seconds'\""  }) -- entries must be comma-separated
-- }}}
...
local theme_path = string.format("%s/.config/awesome/themes/%s", os.getenv("HOME"), chosen_theme)

if not beautiful.init(string.format("%s/my-theme.lua", theme_path)) then
  naughty.notify({ preset = naughty.config.presets.critical,
                     title = "Oops, there were errors in setup of theme!",
                     })
...
clientkeys = my_table.join(
    ...
    awful.key({ modkey, "Shift" }, "s", function (c) c.sticky = not c.sticky end, { description = "toggle on all tags", group = "client"}),
    awful.key({ modkey,           }, "n",
        function (c)
...
awful.rules.rules = {
    ...
    { rule = { class = "git-cola" }, properties = { floating = true } },
    { rule = { class = "calibre" }, properties = { floating = true } },
    { rule = { class = "meld" }, properties = { floating = true } },
    { rule = { class = "google play music desktop player" }, properties = { floating = true } },
